...
@section('content')
    ...
    <div class="mt-[134px]">
        <h1 class="text-neutral-50 text-4xl font-black ">UPDATE ADMIN</h1>
        <div>
            <div class="mt-12">
                <form method="POST" action="{{ route('admin.admin.update', ['id' => $admin->id]) }}">
                    @csrf
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div>
                            <label for="" class="text-neutral-50 text-2xl font-black">Name</label>
                            <input type="text" placeholder="Name" value="{{ $admin->name }}" name="name"
                        ...
                        <div>
                            ...
                            <input type="email" placeholder="Email" value="{{ $admin->email }}" name="email"
                                class="w-full bg-[#383838]  py-4 px-4 text-white outline-none border-none rounded-2xl mt-5">
                            @error('email')
                                <span style="color: red">{{ $message }}</span>
                            @enderror
